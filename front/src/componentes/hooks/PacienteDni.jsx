...
export const usePacienteData = (setValue) => { 
  ...
  useEffect(() => {
    ...
    const handleBlur = async (e) => {
      ...
      try {
        const response = await apiFetch(`/api/pacientes/${dni}`);
        const result = await response.json();

        if (!result.error && result.body.length > 0) {
          const datosPaciente = result.body[0];
          ...
          setValue('fecha_nacimiento', datosPaciente.fecha_nacimiento.split('T')[0]);
    ...
    return () => {
      if (dniInput) {
        dniInput.removeEventListener('blur', handleBlur);
      }
    };
  }, [setValue]);
